<template>
  <v-app>
    ...
    <template v-if="route.name">
      <!-- Top bar -->
      <v-app-bar v-if="isFullLayout" density="comfortable" flat>
        <v-app-bar-nav-icon @click="drawer = !drawer" />
        <div class="d-flex align-center" style="gap: 12px;">
          <div class="ai-core-orb d-flex align-center justify-center" style="width: 32px; height: 32px; background: linear-gradient(135deg, #00F2FF, #0077B6);">
            <v-icon size="18" color="white">mdi-robot</v-icon>
          </div>
          <v-app-bar-title><span class="font-weight-bold">Zalo</span><span style="color: #00F2FF;">CRM</span></v-app-bar-title>
        </div>
        <v-spacer />
        <div class="d-flex align-center mr-4 px-3 py-1 rounded-pill" style="background: rgba(76,175,80,0.1); border: 1px solid rgba(76,175,80,0.2);">
      ...
      <!-- Sidebar -->
      <v-navigation-drawer v-if="isFullLayout" v-model="drawer" :rail="rail" permanent @click="rail = false">
        <v-list density="compact" nav class="mt-2">
          <v-list-item v-for="item in menuItems" :key="item.path" :to="item.path" :prepend-icon="item.icon" :title="item.title" rounded="xl" class="mb-1 mx-2" />
        </v-list>
      </v-navigation-drawer>
